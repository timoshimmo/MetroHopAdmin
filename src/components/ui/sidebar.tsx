...
>(
  (
    ...
    return (
      <aside
        ref={ref}
        className={cn("group/sidebar hidden flex-col border-r bg-background text-sidebar-foreground md:flex",
  ...
  (
    ...
    const hasSubMenu = React.Children.toArray(children).some(
      (child) =>
        ...
        (child.props.className?.includes('group-data-[state=open]:rotate-180') || child.props.className?.includes('group-data-\\[state\\=open\\]:rotate-180'))
    );
    
    const Comp = hasSubMenu ? CollapsibleTrigger : asChild ? Slot : "button";
    
    const buttonContent = (
      <>
        {children}
        <span className={cn("truncate transition-opacity duration-200", state === "collapsed" ? 'opacity-0 w-0' : 'opacity-100 w-auto')}>
          {React.Children.map(children, child => typeof child === 'string' ? child : null)}
        </span>
      </>
    );
    ...
    const button = (
      <Comp
        ...
        className={cn(sidebarMenuButtonVariants({ variant, size }), 'group-data-[state=expanded]/sidebar:gap-x-2', className)}
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn("rounded-md h-8 flex gap-2 items-center",
    ...
    >
      ...
      <Skeleton
        className="h-4 flex-1 max-w-[--skeleton-width] transition-all duration-200"
  ...
  return (
    <CollapsibleContent asChild>
      <ul
        ...
        className={cn(
          "ml-5 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border/50 pl-4 py-1",
